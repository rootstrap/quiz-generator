...
class UploadFile(Page):
    def render(self, app):
        ...
        st.markdown("""Generate a quiz automatically from the content of your the course""")
...
class ConfigureExam(Page):
    def render(self, app):
        ...
        app.set_question_args('number_of_open_questions', st.number_input(
                "Number of questions",
                min_value=0,
                value=30,
        ...
        # ))
        app.set_question_args('number_of_open_questions_exam', st.number_input(
        ...
        app.set_question_args('number_of_exams', st.number_input(
        ...
        with col1:
            if st.button("Generate", help="Generate the questions according to the parameters"):
                if not app.question_args['number_of_open_questions_exam']*app.question_args['number_of_exams']<= app.question_args['number_of_open_questions']:
                    st.error('The number of total questions shoud be bigger than number of questions per exam * number of exams')
                else:
                    with st.spinner('Generating questions. This may take a while...'):
                        try:
                            ...
                            app.open_questions = get_open_questions(content, 
                    ...
                    if  len(app.open_questions)>0:
                        ...
                        generate_exams(open_questions = app.open_questions, 
                                    number_of_open= app.question_args["number_of_open_questions_exam"],
...
class ConfigureMultipleChoice(Page):
    def render(self, app):
        ...
        app.set_question_args('number_of_mc_questions', st.number_input(
            "Number of questions",
            min_value=0,
            max_value=30,
            value=5,
        ...
        app.set_question_args('number_of_answers', st.number_input(
        ...
        app.set_question_args('number_of_mc_questions_exam', st.number_input(
